    ...
    () => {
      ...
      mm.add(MQ.desktop, () => {
        ...
        st.current = ScrollTrigger.create({
          ...
          pin: true,
          snap: { snapTo: 1 / last, duration: { min: 0.2, max: 0.5 }, ease: "power2.inOut", delay: 0.05 },
          onUpdate(self) {
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <li key={s.n} className="relative border-b border-hair">
                    ...
                    <span aria-hidden className="absolute -left-4 top-0 hidden h-full w-px bg-hair lg:block">
                    ...
                    >
                      ...
                      <span
                        className={cn(
                          ...
                          on ? "text-foreground" : "text-foreground lg:text-foreground/35 lg:group-hover:text-foreground/70",
                        )}
                      >
                        {s.title}
                      </span>
                      <span className="hidden font-mono-eyebrow text-ink-faint sm:block">{s.eyebrow}</span>
                    </button>
                    <div className={cn("step-body grid transition-[grid-template-rows] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]", on ? "lg:grid-rows-[1fr]" : "lg:grid-rows-[0fr]")}>
                      <div className="overflow-hidden">
                        <p className="max-w-md pb-6 pl-[72px] leading-relaxed text-muted-foreground">{s.body}</p>
            ...
            <div aria-hidden className="relative hidden lg:block">
              <div className="relative h-full min-h-[380px] overflow-hidden rounded-2xl border border-hair bg-card/60 backdrop-blur-xl">
                <div className="flex items-center justify-between border-b border-hair px-5 py-3">
                  ...
                  <span className="font-mono text-[11px] text-ink-faint">step-{steps[active].n}.log</span>
                </div>
                ...
                {steps.map((s, i) => (
                  <div
                    ...
                    className={cn(
                      ...
                      active === i ? "translate-y-0 opacity-100" : active > i ? "-translate-y-6 opacity-0" : "translate-y-6 opacity-0",
                    )}
                  >
                    ...
                    <ul className="space-y-3 font-mono text-[13px]">
                      {s.log.map((l, k) => (
                        <li key={l} className="grid grid-cols-[70px_1fr] gap-3 border-t border-hair pt-3">
